# Как настроить GitHub Action
В этом руководстве вы узнаете, как настроить GitHub Action для автоматического перевода содержимого вашего репозитория с помощью Frenglish SDK.

## Предварительные требования

1. Репозиторий на GitHub, в котором вы хотите реализовать переводы
2. Приватный API-ключ Frenglish (получите его на [www.frenglish.ai](http://www.frenglish.ai) в разделе настроек разработчика вашего проекта cli/sdk)

## Этапы настройки

1.  **Создайте файл рабочего процесса GitHub Action**

    Создайте новый файл в вашем репозитории по пути `.github/workflows/frenglish-translation.yml` и вставьте в него предоставленный код GitHub Action.
...
2.  **Настройте API-ключ Frenglish**

    Сохраните ваш API-ключ Frenglish как секрет GitHub:

    a. Перейдите в ваш репозиторий на GitHub
    b. Нажмите "Settings" > "Secrets and variables" > "Actions"
    c. Нажмите "New repository secret"
    d. Имя: `FRENGLISH_API_KEY`
    e. Значение: ваш приватный API-ключ Frenglish
    f. Нажмите "Add secret"

3.  **Создайте скрипт перевода**

    Создайте новый файл по пути `.github/scripts/translate.js`. Этот скрипт будет использовать Frenglish SDK для выполнения перевода. Вот базовый пример:
    ...
    При необходимости адаптируйте этот скрипт под ваши конкретные требования к переводу.

4.  **Обновите ваш `package.json`**

    Убедитесь, что ваш `package.json` содержит Frenglish SDK в списке зависимостей:
...
5.  **Закоммитьте и отправьте изменения**
    ...
    Добавьте новые файлы в ваш репозиторий:
...
1. Действие запускается при каждом пуше в любую ветку.
2. Он клонирует ваш код и настраивает Node.js.
3. Устанавливает необходимые зависимости, включая Frenglish SDK.
4. Запускает ваш скрипт перевода, который использует Frenglish SDK для перевода файлов.
5. Если были внесены изменения, автоматически создаются файлы локализации на поддерживаемых языках в указанной вами директории перевода.

## Настройка

- Вы можете изменить триггер в файле рабочего процесса, чтобы запускать его на определённых ветках или событиях.
- Измените скрипт `translate.js`, чтобы обрабатывать разные типы файлов или процессы перевода по мере необходимости.
- Обновите шаг создания pull request, если хотите изменить способ отправки переводов на проверку.

## Устранение неполадок
Если возникли проблемы:
1. Проверьте журналы Action на вкладке GitHub Actions на наличие сообщений об ошибках.
2. Убедитесь, что ваш приватный API-ключ Frenglish правильно задан в секретах репозитория.
3. Проверьте, что ваш скрипт `translate.js` корректно использует Frenglish SDK.

Не забывайте держать ваш API-ключ Frenglish в секрете и никогда не коммитьте его напрямую в репозиторий.